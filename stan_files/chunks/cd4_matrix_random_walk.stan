...
data {
 ...
 matrix[time_steps_euler  , time_steps_year  ] X_design;                // This is our spline design matirx that we are modelling kappa with. 
...
transformed parameters{
 ...
 y_hat = simpleepp_no_art( X_design * beta , iota, mu, sigma, mu_i, dt_2)[rows_to_interpret, ];
}
...
model {
 ...
 target += normal_lpdf( D_penalty * beta | 0, sigma_pen);                // So this models our penalized spline with a slightly altered distribution command
  ...
  for( i in 1:n_obs){
  y[i] ~ binomial(n_sample, y_hat[i , 3]);                               // This fits it to the binomially sampled data 
  }
...
fitted_output = simpleepp_no_art(X_design * beta, iota, mu, sigma, mu_i, dt_2);       // This models the data after our fitting procedure
