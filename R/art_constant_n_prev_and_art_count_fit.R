...
expose_stan_functions("C:/Users/josh/Dropbox/hiv_project/simpleepp/stan_files/chunks/ART_count_and_prevalence_fitter.stan")
...
sim_art_epidemic<-function(kappa_params,params){
  ...
  test_run<-simpleepp_art_prev_and_count(kappa = kappa,iota = iota,alpha = alpha,mu = mu,sigma = sigma, mu_i = mu_i, mu_d = mu_d,mu_a = mu_a,
...
diag_start<- 1982                                                      ## start of diagnosis
art_start<-1996                                                       ## start of ART
...
count_rows<-seq((art_start-start)*10+1,(year_seq[length(year_seq)]-start+1)*10)


stan_data_discrete_prev_rw_count<-list(
  n_obs = length(year_seq),
  y = as.array(poisson_sampled_data$sample_data$sampled),
  n_sample = sample_n,
  n_obs_prev = sample_years,
  y_prev = sample_df_prev$sample_y_hiv_prev,
  knot_number = ncol(spline_matrix),
  ...
  mu_i = mu_i,
  dt_2 = 0.1,
  prev_rows = as.array(rows_to_evaluate),
  ...
  art_prog = art_prog,
  onem = obem,
  rows_to_interpret = count_rows
  
)

count_and_prev_fitter<-stan_model("C:/Users/josh/Dropbox/hiv_project/simpleepp/stan_files/chunks/ART_count_and_prevalence_fitter.stan")
optimized_count_and_prev<-optimizing(count_and_prev_fitter,stan_data_discrete_prev_rw_count,as_vector=F)
...
a<-optim_plotter(art_epidemic$df,optimized_count_and_prev)
a$prev_plot
a$inc_plot
a$kappa_plot
a$art_plot
