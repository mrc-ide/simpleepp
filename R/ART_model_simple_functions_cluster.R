count_data_fitting_spline<-function(simulated_dataset,stan_data){
...
prev_data_fitting_spline<-function(simulated_data,stan_data){
  rstan::rstan_options(auto_write = TRUE)                               ## Need these options like this for parrallel going
  options(mc.cores = parallel::detectCores())
...
results<-plot_stan_model_fit(mod_hiv_prev)

return(results)
}

prev_data_fitting_RW<-function(simulated_data,stan_data){
  rstan::rstan_options(auto_write = TRUE)                               ## Need these options like this for parrallel going
  options(mc.cores = parallel::detectCores())
...
count_data_fitting_RW<-function(simulated_data,stan_data){
  rstan::rstan_options(auto_write = TRUE)                               ## Need these options like this for parrallel going
  options(mc.cores = parallel::detectCores())
