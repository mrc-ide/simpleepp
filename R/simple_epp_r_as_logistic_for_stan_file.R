#' This script will implement a simple EPP model, with CD4 structure and an r paramter varying through time based on a logistic model 
...
hiv_ode_model<-function(init_conditions,params,times_in_model){
  ...
  s0<- init_conditions$n_init -(cd4_1+cd4_2+cd4_3+cd4_4)      ## Initial susceptible proportion of the population
  ...
  eta<- params$eta    #params$eta                                ## The birth rate into the population
  #kappa<- runif(101,min = 0,max = 1) #rep(0.3,101) #params$kappa                                ## The transmission rate paramter
  ...
  ################################################################################################################################
  ## NOw we will model the chaning kappa term over time ##########################################################################
  ################################################################################################################################
  
  rlogistic <- function(t, p) {                                  ## So rlogistic is our function to create the logistic curve of kappa
    p[1] - (p[1] - p[2]) / (1 + exp(-p[3] * (t - p[4])))
  }
  
  
  theta0 <- c(log(params$kappa[1]), log(params$kappa[2]), params$kappa[3], params$kappa[4] )
  times_log<-times_in_model[1]:times_in_model[2]
  
 
  kappa<-exp(rlogistic(t= times_log,p = theta0))
  ...
  t_min<-times_in_model[1]
  t_max<-times_in_model[2]
  times<-t_min:t_max
  ...
  return(list(sample_df=out_epp_cd4_hiv,inits_length=length(inits_hiv_cd4),kappa=kappa))
  
}

inits_for_model<-list(cd4=c(100,0,0,0),n_init=1000000)
params_for_model<-list(eta=2000,kappa=c(0.5,0.1,0.2,25),mu_i=c(0.003, 0.008, 0.035, 0.27),sigma=1/c(3.16, 2.13, 3.20),mu_s=1/35)
times_for_model<-c(0,100)

out_epp_cd4_hiv<-hiv_ode_model(init_conditions = inits_for_model,params = params_for_model,times_in_model = times_for_model)
################################################################################################################################
...
stan_d_hiv_prev = list(n_obs = sample_years,
                       n_params = 5,
                       n_difeq =  10, #out_epp_cd4_hiv$inits_length,
                       n_sample = sample_n,
...
test_hiv_100_year = stan("stan_files/chunks/cd4_r_logistic_model.stan",
...
plot_stan_model_fit<-function(model_output,sampled_df,plot_name,stan_data){
  ...
  params_low<-apply(posts_hiv$params,2,quantile,probs=c(0.025))
  params_high<-apply(posts_hiv$params,2,quantile,probs=c(0.975))
  
  params_df<-rbind.data.frame(params_low,params,params_high)
  names(params_df)<-c("eta","alpha","beta","gamma","midway")
  ...
  prev_median<-(apply(posts_hiv$fake_I[,,9],2,median))*100
  prev_low<-(apply(posts_hiv$fake_I[,,9],2,quantile,probs=c(0.025)))*100
  prev_high<-(apply(posts_hiv$fake_I[,,9],2,quantile,probs=c(0.975)))*100
  
  
  incidence_median<-apply(posts_hiv$fake_I[,,10],2,median)
  incidence_low<-apply(posts_hiv$fake_I[,,10],2,quantile,probs=c(0.025))
  incidence_high<-apply(posts_hiv$fake_I[,,10],2,quantile,probs=c(0.975))
  
  r_median<-apply(posts_hiv$fake_I[,,1],2,median)
  r_low<-apply(posts_hiv$fake_I[,,1],2,quantile,probs=c(0.025))
  r_high<-apply(posts_hiv$fake_I[,,1],2,quantile,probs=c(0.975))
  ...
  r_fit<-data.frame(r_low,r_median,r_high,stan_data$fake_ts)
  ...
  return(list(prevalence_plot=(plotter),param_values=params_df,inits=inits,df_output=df_fit_prevalence,incidence_df=df_fit_incidence,
              r_fit_df=r_fit,incidence_plot=incidence_plot,r_plot=r_plot))
